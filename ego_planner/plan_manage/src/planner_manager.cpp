...
namespace ego_planner
{
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    bspline_optimizer_->setLocalTargetPt( local_target_pt );    // 设置局部轨迹终点

    ros::Time t_start = ros::Time::now();       // 记录开始时间
    ros::Duration t_init, t_opt, t_refine;

    /*** STEP 1: INIT ***/
    // 设置持续时间，离终点比较远，轨迹时间短一点；离终点比较近，轨迹时间长一点
    // 时间间隔分段
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        double dist = (start_pt - local_target_pt).norm();      // 起点到局部轨迹终点的距离
        // 分配的时间为以最大加速度和速度飞行所需的时间
        ...
        else // 在始末位置之间随机插入一个点，再计算轨迹
        {
          // 叉乘求方向
          Eigen::Vector3d horizen_dir = ((start_pt - local_target_pt).cross(Eigen::Vector3d(0, 0, 1))).normalized();    // 水平方向
          Eigen::Vector3d vertical_dir = ((start_pt - local_target_pt).cross(horizen_dir)).normalized();                // 垂直方向
          ...
          pos.col(0) = start_pt;        // 起点
          pos.col(1) = random_inserted_pt;  // 随机插入的点
          pos.col(2) = local_target_pt;     // 局部轨迹终点
          Eigen::VectorXd t(2);
          t(0) = t(1) = time / 2;
        ...
        do
        {
          ...
          point_set.clear();
          flag_too_far = false;
          Eigen::Vector3d last_pt = gl_traj.evaluate(0);      // 多项式轨迹起点
          for (t = 0; t < time; t += ts)
          {
            Eigen::Vector3d pt = gl_traj.evaluate(t);
            // 如果通过时间间隔分段得到的距离大于1.5倍控制点的距离，则ts /= 1.5，再继续分
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        vector<double> pseudo_arc_length;         // 分段距离
        vector<Eigen::Vector3d> segment_point;    // 分段点
        pseudo_arc_length.push_back(0.0);
        for (t = t_cur; t < local_data_.duration_ + 1e-3; t += ts)
        {
          segment_point.push_back(local_data_.position_traj_.evaluateDeBoorT(t));       // 求出B样条曲线的坐标
          if (t > t_cur)
          {
        ...
        start_end_derivatives.push_back(local_target_vel);                                      // 局部目标点速度
        start_end_derivatives.push_back(local_data_.acceleration_traj_.evaluateDeBoorT(t_cur)); // 加速度
    ...
    UniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts);      // 得到控制点
    ...
    {
      // cout << "enter" << endl;
      std::vector<ControlPoints> trajs = bspline_optimizer_->distinctiveTrajs(segments);      // 返回多个安全的控制点集
      cout << "\033[1;33m"
      ...
      {
        if (bspline_optimizer_->BsplineOptimizeTrajRebound(ctrl_pts_temp, final_cost, trajs[i], ts))      // 优化控制点集
        {
    ...
    {
      flag_step_1_success = bspline_optimizer_->BsplineOptimizeTrajRebound(ctrl_pts, ts);     // 直接优化控制点
      t_opt = ros::Time::now() - t_start;
      //static int vis_id = 0;
    ...
    UniformBspline pos = UniformBspline(ctrl_pts, 3, ts);     // 控制点，阶数，时间间隔
    pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, pp_.feasibility_tolerance_);      // 设置动力学约束
