...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    planner_manager_->deliverTrajToOptimizer();                 // store trajectories 存储路径
    planner_manager_->setDroneIdtoOpt();                        // 设置无人机id

    /* callback */
    // 定时器
    exec_timer_ = nh.createTimer(ros::Duration(0.01), &EGOReplanFSM::execFSMCallback, this);    // 状态回调函数
    safety_timer_ = nh.createTimer(ros::Duration(0.05), &EGOReplanFSM::checkCollisionCallback, this);   // 检查障碍物回调

    odom_sub_ = nh.subscribe("odom_world", 1, &EGOReplanFSM::odometryCallback, this);       // 接收里程计消息
    state_sub_ = nh.subscribe("/mavros/state", 1, &EGOReplanFSM::stateCallback, this);          // 接收无人机状态
    ...
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)    // rviz设置目标点
    {
  ...
  void EGOReplanFSM::stateCallback(const mavros_msgs::StateConstPtr &msg)
  {
    currentState = *msg;
  }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case REPLAN_TRAJ:       // 轨迹重新规划
    {
      ...
      {
        changeFSMExecState(REPLAN_TRAJ, "FSM");     // 一直重新规划
      }
    ...
    case EXEC_TRAJ:         // 
    {
      ...
      else if ((local_target_pt_ - end_pt_).norm() < 1e-3) // close to the global target 局部轨迹终点靠近全局终点
      {
        if (t_cur > info->duration_ - 1e-2)
        {
          ...
          changeFSMExecState(WAIT_TARGET, "FSM");       // 等待下一个目标点
          goto force_return;
          // return;
        }
        else if ((end_pt_ - pos).norm() > no_replan_thresh_ && t_cur > replan_thresh_)
        {
          changeFSMExecState(REPLAN_TRAJ, "FSM");       // 重新规划
        }
      }
    ...
    case EMERGENCY_STOP:        // 应急停止
    {

      if (flag_escape_emergency_) // Avoiding repeated calls
      {
        callEmergencyStop(odom_pos_);       // 调用应急停止
      }
      else
      {
  ...
  bool EGOReplanFSM::planFromGlobalTraj(const int trial_times /*=1*/) //zx-todo
  {
    ...
    bool flag_random_poly_init;     // 随机多项式标志
    if (timesOfConsecutiveStateCalls().first == 1)      // 返回状态机在该状态的停留次数
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_and_refine_success)      // 规划成功
    {
      ...
      bspline.pos_pts.reserve(pos_pts.cols());      // 开辟存储区
  ...
  void EGOReplanFSM::getLocalTarget()
  {
    ...
    double t_step = planning_horizen_ / 20 / planner_manager_->pp_.max_vel_;    // 时间步长
    double dist_min = 9999, dist_min_t = 0.0;
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      Eigen::Vector3d pos_t = planner_manager_->global_data_.getPosition(t);
      double dist = (pos_t - start_pt_).norm();   // 与起点的距离
      ...
      // 在全局轨迹global_data_找到距离当前无人机位置planning_horizen_处的点作为局部轨迹的目标点，并将全局轨迹上离当前位置最近的点对应的时间赋值给last_progress_time
    ...
    // 如果局部轨迹的目标点对应的时间大于全局轨迹的时长，那么将全局终点作为局部轨迹的目标点
    ...
    // 位移公式
    // 如果全局终点到局部轨迹的目标点的距离小于无人机以最大速度和加速度飞行的距离，则局部轨迹的目标点速度设为0
    ...
    // 否则，局部目标点需要设置速度，因为这段局部轨迹的目标点是下一段轨迹的开始点
    else
    {
