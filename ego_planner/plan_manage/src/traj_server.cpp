...
void bsplineCallback(traj_utils::BsplineConstPtr msg)
{
  ...
  traj_.push_back(traj_[0].getDerivative());    // 一阶导
  traj_.push_back(traj_[1].getDerivative());    // 二阶导
...
std::pair<double, double> calculate_yaw(double t_cur, Eigen::Vector3d &pos, ros::Time &time_now, ros::Time &time_last)
{
  ...
  constexpr double YAW_DOT_MAX_PER_SEC = PI;        // 每秒最大转向
  // constexpr double YAW_DOT_DOT_MAX_PER_SEC = PI;
  std::pair<double, double> yaw_yawdot(0, 0);
  double yaw = 0;
  double yawdot = 0;
  // 在轨迹持续时间内则用当前位置指向提前的位置的方向，否则用当前位置指向轨迹终点的方向
  Eigen::Vector3d dir = t_cur + time_forward_ <= traj_duration_ ? traj_[0].evaluateDeBoorT(t_cur + time_forward_) - pos : traj_[0].evaluateDeBoorT(traj_duration_) - pos;
  double yaw_temp = dir.norm() > 0.1 ? atan2(dir(1), dir(0)) : last_yaw_;     // 航向就是绕z轴旋转
  ...
  else
  {
    ...
    if (yaw_temp - last_yaw_ < -max_yaw_change)
    {
      ...
      yawdot = -YAW_DOT_MAX_PER_SEC;    // 航向速率
    }
    // 大于最大允许角度
    // 因为不能大于这一时间间隔最大的转向角度，所以只能加最大转向角度
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  {
    // Deboor-Cox递推公式求出u时刻B样条曲线的位置，速度和加速度信息
...
int main(int argc, char **argv)
{
  ...
  nh.param("traj_server/time_forward", time_forward_, -1.0);      // 提前的时间
  last_yaw_ = 0.0;
  last_yaw_dot_ = 0.0;
