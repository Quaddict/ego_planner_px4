...
namespace ego_planner
{
  ...
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
    ...
    control_pts_buf.reserve(MAX_TRAJS);       // 请求空间使用权，与容器capacity有关
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      RichInfoSegs.push_back(RichInfoOneSeg);       // first是正向，second是反向
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      exit_multi_loop1:;  
        for (int j = RichInfoSegs[i].first.size - 1; j >= 1; j--)     // 从后往前
        {
          //cout << "j=" << j << endl;
          //cout << "B *" << j << "*" << endl;
          ;
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          base_vec_reverse = -RichInfoSegs[i].first.direction[j][0];      // 反向矢量
          ...
          if (grid_map_->getInflateOccupancy(base_pt_reverse)) // Search outward. 向外搜寻
          {
            ...
            for (; l <= l_upbound; l += RESOLUTION)
            {
              ...
              if (!grid_map_->getInflateOccupancy(base_pt_temp))      // 扩展到无障碍物区域，记录牵引点
              {
            ...
            if (l > l_upbound)    // 5倍控制点距离都没跳出障碍物，则清除该段控制点对
            {
          ...
          // 不在障碍物点，且反向牵引点与控制点的距离大于栅格大小，则直接记录反向牵引点和反向牵引方向
        ...
        // occ_start_id之前和occ_end_id之后的点设置成跟他俩一样，occ_start_id和occ_end_id不一定和障碍物控制点一致
    ...
    selection[0] = -1; // init
    int max_traj_nums = static_cast<int>(pow(VARIS, seg_upbound));    // 2的x次方
    for (int i = 0; i < max_traj_nums; i++)
    {
      // 2.1 Calculate the selection table.
      // 计算选择表，这是用来选择哪一段曲线是正向或者反向，如有有两段曲线，选择表为(0,0)，则说明两段曲线都选择正向；(1,0)说明第一段曲线是选择反向，第二段曲线选择正向
      int digit_id = 0;
      selection[digit_id]++;
      ...
      while (/*seg_id < RichInfoSegs.size() ||*/ cp_id < cps_.size)
      {
        ...
        if (seg_id >= seg_upbound || cp_id < segments[seg_id].first || cp_id > segments[seg_id].second)   // 障碍物控制点外的控制点保持原来的控制点
        {
        ...
        else if (cp_id >= segments[seg_id].first && cp_id <= segments[seg_id].second)   // 障碍物控制点内的控制点
        {
          // 正向
    ...
    return control_pts_buf;     // 控制点集合存储
  } // namespace ego_planner
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    if (flag_first_init)
    {
      ...
      cps_.points = init_points;        // 原始控制点
    }

    /*** Segment the initial trajectory according to obstacles ***/
    // 对越过障碍物的初始轨迹进行分段
    constexpr int ENOUGH_INTERVAL = 2;        // 足够的间隔
    ...
    // 检测在障碍物中的控制点，找出在其两边不在障碍物中的控制点，以此作为这一段轨迹的起点终点
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      for (double a = 1.0; a > 0.0; a -= step_size)
      {
        ...
        // occ = 1为是障碍物
        if (occ && !last_occ) // 当前点是障碍物，上一点不是障碍物
        {
        ...
        else if (!occ && last_occ)    // 当前点不是障碍物，上一点是障碍物
        {
          out_id = i;
    ...
    // 为每段分配数据
    for (size_t i = 0; i < segment_ids.size(); i++)     // 对于每一段调整后的障碍物控制点对
    {
      // step 1     对于每个控制点对中的控制点
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        // A*规划的路径中点和控制点构成的向量，与该控制点前后两点构成的向量进行点积，从而判断这两个向量的夹角
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          // 因为要近似垂直，所以当前需要一边大于90，另一边小于90，即值乘以上一次的值要小于0
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
          {
            // 找出一个与控制点近似垂直的点，计算出两个向量在同一条线上投影的比例，以此确定近似垂直的点
            intersection_point =
            ...
            got_intersection_id = j;      // 当前进入障碍物的控制点的索引
            break;
          }
        }
        ...
        {
          double length = (intersection_point - init_points.col(j)).norm();   // 牵引点和控制点的距离
          if (length > 1e-5)
          {
            cps_.flag_temp[j] = true;
            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
            {
              // 从牵引点向控制点靠近，直到找到在障碍物表面的点
              ...
              if (occ || a < grid_map_->getResolution())
              {
                ...
                cps_.base_point[j].push_back((a / length) * intersection_point + (1 - a / length) * init_points.col(j));    // 障碍物表面但不在障碍物上的点
                cps_.direction[j].push_back((intersection_point - init_points.col(j)).normalized());                        // 控制点指向牵引点的方向
                // cout << "A " << j << endl;
                break;
              }
            }
      ...
      if (segment_ids[i].second - segment_ids[i].first == 1)
      {
        ...
        Eigen::Vector3d middle_point = (init_points.col(segment_ids[i].second) + init_points.col(segment_ids[i].first)) / 2;    // 中点
        int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
      ...
      //step 3
      // 如果障碍物控制点对之间有控制点没有牵引点和牵引方向，则将上一个牵引点和方向作为牵引点和方向
      if (got_intersection_id >= 0)         // 得到牵引点的索引
      {
        for (int j = got_intersection_id + 1; j <= adjusted_segment_ids[i].second; ++j)
          if (!cps_.flag_temp[j])
          {
            cps_.base_point[j].push_back(cps_.base_point[j - 1].back());      // 经过step2会产生多个牵引点
    ...
    return final_segment_ids;     // 返回最终的障碍物控制点
  }
  ...
  double BsplineOptimizer::costFunctionRebound(void *func_data, const double *x, double *grad, const int n)
  {
    ...
    double cost;
    opt->combineCostRebound(x, grad, cost, n);    // 求得目标函数
  ...
  {
    setBsplineInterval(ts);       // 设置样条曲线时间间隔
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    {
      /* ---------- prepare ---------- */
      min_cost_ = std::numeric_limits<double>::max();     // 返回double数据类型的最大值
      ...
      /**
       * 用于内存复制的函数 void *memcpy(void *destin, void *source, unsigned n)
       * 作用是:以source指向的地址为起点，将连续的n个字节数据，复制到destin指向的地址为起点的内存中*/ 
      memcpy(q, cps_.points.data() + 3 * start_id, variable_num_ * sizeof(q[0]));     

      lbfgs::lbfgs_parameter_t lbfgs_params;        // 参数
      lbfgs::lbfgs_load_default_parameters(&lbfgs_params);    
      lbfgs_params.mem_size = 16;                 // 存储最近16次的迭代数据
      lbfgs_params.max_iterations = 200;          //最大迭代次数
      lbfgs_params.g_epsilon = 0.01;              // 运行误差
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    calcSmoothnessCost(cps_.points, f_smoothness, g_smoothness);            // 光滑项
