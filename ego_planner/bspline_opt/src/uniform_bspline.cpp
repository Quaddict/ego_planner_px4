...
namespace ego_planner
{
  ...
  // 设置控制点control_points_， 阶数p_， 时间间隔interval_
                                         ...
                                         const double &interval)
  {
    ...
    u_ = Eigen::VectorXd::Zero(m_ + 1);       // 节点向量
    // 节点向量赋值，可能是均匀B样条曲线
    for (int i = 0; i <= m_; ++i)
    {
  ...
  // 先定位u在哪个区间内，然后使用Deboor-Cox递推公式求出u时刻B样条曲线的三维坐标
  Eigen::VectorXd UniformBspline::evaluateDeBoor(const double &u)
  {
    ...
    for (int i = 0; i <= p_; ++i)
    {
      d.push_back(control_points_.col(k - p_ + i));     // 控制点集合
    ...
    for (int r = 1; r <= p_; ++r)
    {
      for (int i = p_; i >= r; --i)
      {
        ...
        d[i] = (1 - alpha) * d[i - 1] + alpha * d[i];     // 相邻控制点之间的点
      }
    }
  ...
  // 得到B样条曲线的导数的控制点后，求得该导数曲线，阶数为原来阶数-1
  UniformBspline UniformBspline::getDerivative()
  {
    ...
    Eigen::VectorXd knot(u_.rows() - 2);      // 节点向量
                                             ...
                                             Eigen::MatrixXd &ctrl_pts)
  {
    ...
    if (start_end_derivative.size() != 4)   // 4个已知条件：起末速度和加速度
    {
    ...
    // write A
    /**
     * 这是根据B样条的递归定义方式，推导出来的一种矩阵表示形式。
     * 这个系数B样条的矩阵形式，因为B样条是递归，所以每一行系数对应不同的n阶导数
     * |1  4  1|  对应0阶导数-位置
     * |-3 0 -3|  对应一阶导数-速度
     * |3 -6  3|  对应二阶导数-加速度
    */
    ...
    for (int i = 0; i < K; ++i)
      A.block(i, i, 1, 3) = (1 / 6.0) * prow.transpose();             // 对应k个轨迹点的位置

    A.block(K, 0, 1, 3) = (1 / 2.0 / ts) * vrow.transpose();          // 对应起点速度
    A.block(K + 1, K - 1, 1, 3) = (1 / 2.0 / ts) * vrow.transpose();  // 对应终点速度

    A.block(K + 2, 0, 1, 3) = (1 / ts / ts) * arow.transpose();       // 对应起点加速度
    A.block(K + 3, K - 1, 1, 3) = (1 / ts / ts) * arow.transpose();   // 对应终点加速度
