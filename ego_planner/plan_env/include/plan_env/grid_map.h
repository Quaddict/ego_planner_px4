...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos) {
  if (!isInMap(pos)) return -1;     // 判断该点位置是否超过地图边界

  Eigen::Vector3i id;
  posToIndex(pos, id);              // 坐标转化为像素地图坐标

  return int(md_.occupancy_buffer_inflate_[toAddress(id)]);   // 像素地图坐标求出在第几个像素，在之前记录的存储区中索引出是否为障碍物
}
