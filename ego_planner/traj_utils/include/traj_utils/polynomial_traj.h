...
public:
  ...
  // 加入一段多项式轨迹，把x,y,z方向上的多项式系数压入cxs，cys,czs，时长压入times
  ...
  Eigen::Vector3d evaluate(double t)
  {
    ...
    /* evaluation */
    int order = cxs[idx].size();        // 阶数
    Eigen::VectorXd cx(order), cy(order), cz(order), tv(order);
    for (int i = 0; i < order; ++i)
    {
      cx(i) = cxs[idx][i], cy(i) = cys[idx][i], cz(i) = czs[idx][i];  // 提取出x,y,z方向上的多项式系数
      tv(order - 1 - i) = std::pow(t, double(i));                     // 计算t^n, t^(n-1),...,t^0
    }

    Eigen::Vector3d pt;
    pt(0) = tv.dot(cx), pt(1) = tv.dot(cy), pt(2) = tv.dot(cz);   // 多项式值是多项式系数和幂次向量的点积
    return pt;
  }

  // 计算t时刻多项式轨迹的速度，原理就是对多项式轨迹进行求导，实际是直接求求导后的系数，然后进行点积
  Eigen::Vector3d evaluateVel(double t)
  {
    ...
    {
      // cxs，cys,czs的储存格式是从左到右，t^3,t^2,t^1,t^0,从高阶到低阶
