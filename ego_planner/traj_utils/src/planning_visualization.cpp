...
namespace ego_planner
{
                                                ...
                                                Eigen::Vector4d color, int id, bool show_sphere /* = true */ )
  {
    ...
    sphere.type = visualization_msgs::Marker::SPHERE_LIST;            // 球形
    line_strip.type = visualization_msgs::Marker::LINE_STRIP;         // 线条
    sphere.action = line_strip.action = visualization_msgs::Marker::ADD;  // 箭头
    sphere.id = id;
  ...
  void PlanningVisualization::displayMultiInitPathList(vector<vector<Eigen::Vector3d>> init_trajs, const double scale)
  {
    ...
    if (init_list_pub.getNumSubscribers() == 0)     // 没有接收的话题
    {
      return;
    }
