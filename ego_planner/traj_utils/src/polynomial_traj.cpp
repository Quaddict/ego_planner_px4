                                           ...
                                           const Eigen::Vector3d &end_acc, const Eigen::VectorXd &Time)
{
  ...
  // 构建多项式轨迹矩阵
  Eigen::MatrixXd Ab;
  Eigen::MatrixXd A = Eigen::MatrixXd::Zero(seg_num * 6, seg_num * 6);    // 存放多段多项式轨迹
  /**
   * |1 0 0   0    0      0     |
   * |1 t t^2 t^3  t^4    t^5   |
   * |0 1 0   0    0      0     |
  ...
  */
  ...
  num_f = 2 * seg_num + 4; // 3 + 3 + (seg_num - 1) * 2 = 2m + 4    已知变量：起点，终点的位置，速度和加速度；中间插入点的位置
  num_p = 2 * seg_num - 2; //(seg_num - 1) * 2 = 2m - 2             未知变量：中间插入点的速度和加速度
  num_d = 6 * seg_num;     // 总的变量个数
  ...
  for (int i = 0; i < poly_coeff.rows(); ++i)
  {
    ...
    double ts = Time(i);
    poly_traj.addSegment(cx, cy, cz, ts);     // 保存多项式系数
  }
                                                    ...
                                                    double t)
{
  ...
  // 这里的取值是对应t=0的情况
  C(0, 5) = 1;      // 多项式轨迹的常数项的系数
  C(1, 4) = 1;      // 多项式轨迹一阶导数的常数项的系数
  C(2, 3) = 2;      // 多项式轨迹二阶导数的常数项的系数
  Crow << pow(t, 5), pow(t, 4), pow(t, 3), pow(t, 2), t, 1;         // 轨迹多项式
  C.row(3) = Crow;
  Crow << 5 * pow(t, 4), 4 * pow(t, 3), 3 * pow(t, 2), 2 * t, 1, 0; // 轨迹多项式一阶导数   速度
  C.row(4) = Crow;
  Crow << 20 * pow(t, 3), 12 * pow(t, 2), 6 * t, 2, 0, 0;           // 轨迹多项式二阶导数   加速度
  C.row(5) = Crow;
  // 记录起点和终点的位置，常数矩阵
  ...
  for (int i = 0; i < 6; i++)
  {
    cx[i] = Cofx(i);        // x方向的轨迹多项式
    cy[i] = Cofy(i);        // y方向的轨迹多项式
    cz[i] = Cofz(i);        // z方向的轨迹多项式
  }

  PolynomialTraj poly_traj;
  poly_traj.addSegment(cx, cy, cz, t);    // 加入一段多项式轨迹，把x,y,z方向上的多项式系数压入cxs，cys,czs，时长压入times
